...
- Check out the Stack Overflow [Web World Wind on StackOverflow](https://stackoverflow.com/questions/tagged/webworldwind)
- Email the [Administrative Contact](mailto:jakub@balhar.net)

## Design and Coding Guidelines

These design and coding guidelines are for **Web WorldWind** extension repository

### General

* The Web World Wind extension repository is written in ECMAScript 6 (ES6) and transpiled via Babel with Webpack. 
* The Module pattern is followed for all functionality. All JavaScript code is captured within a module. ES6 module functionality is used to support the Module pattern. The babel and webpack are used to transpile the code to the ES5 compatible browsers. 
* Variable and function names are clear, descriptive and easy to read. They are not labelled with a convention not described in this document. Use whole words rather than abbreviations, e.g., "index" rather than "idx". Correct all variable spelling warnings flagged by lint. 
...
* All available extension modules are exposed via src/index.js where they are exported as part of the default export object under the names of the classes. 

### Exceptions

* Pass exceptions through to the application unless there's good reactive/corrective behavior that can be applied within extension.
* Log any exceptions prior to throwing them. Use the same message for the log as for the exception. 
* Functions validate their arguments and throw the appropriate exception, typically ArgumentError, and identify the exception message the parameter name and the problem -- null, out of range, etc. Validation is a necessary part of the implementation; code should not be checked in without it.
...
* Run JSDoc and review the results prior to checking in any code. It's a convenient check on documentation and typing. Set up npm and run the jsdoc target: npm run jsdoc. 
* Document all functions and properties. Code isn't complete until the documentation is written. Write all documentation when you implement the function or property. Don't wait until "later". Assume that you will never return to a module to "clean up".
...
This section guides you through submitting a bug report to Web WorldWind extensions. Following these guidelines helps both the team of the extensions repository and community understand your report, reproduce the behavior, and find related reports.

#### Before Submitting a Bug Report

* Check the [**StackOverflow**](https://stackoverflow.com/questions/tagged/webworldwind).
* Check this repository's [**issues**](https://github.com/balhar-jakub/webworldwind-x/issues) to see if the problem has
...
Explain the problem and include additional details to help the extension repository team reproduce the problem:
...
Before creating new feature suggestions, check this repository's [issues](https://github.com/balhar-jakub/webworldwind-x/issues)
...
For resources related to the Web WorldWind, please check out the website: https://worldwind.arc.nasa.gov/.

For community support and FAQs, check out the Stack Overflow: https://stackoverflow.com/questions/tagged/webworldwind.

To reach our Administrative Contact, please email: [jakub@balhar.net](mailto:jakub@balhar.net).
